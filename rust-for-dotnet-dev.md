...
[`cfg macro`][cfg-macro] to control [conditional compilation][conditional-compilation]
...
`ConfigurationPredicate` (more on available predicates in
[conditional compilation docs][conditional-compilation])

```rust
// The function is only included in the build when compiling for aarch64
[cfg(target_arch = "aarch64")]
fn aarch64_only() {
  // ...
}

// This function is only included when the OS is not unix
#[cfg(not(unix))]
fn needs_windows() {
  // ...
}
